...
if __name__ == "__main__":
    sec_edgar_obj = sec_edgar_endpoint()
    ticker = "MCD"
    cik = sec_edgar_obj.get_cik_matching_ticker(ticker)
    data = sec_edgar_obj.get_submission_data()
    filtered_filings = sec_edgar_obj.get_filtered_filings_data()
    trailing_data = sec_edgar_obj.get_trailing_data()

    #print(trailing_data)

trailing_data['reportDate'] = pd.to_datetime(trailing_data['reportDate'])

# --- You need to add the Ticker information. ---
# For demonstration, let's assume the CIK 0000063908 corresponds to MCD (McDonald's)
# In a real scenario, you might have a separate mapping or fetch this information.
trailing_data['ticker'] = sec_edgar_obj.ticker.lower() # This is a placeholder, replace with your actual ticker logic
...
print(trailing_data[['accessionNumber', 'reportDate', 'ticker', 'edgar_link']].to_string())
...
finacial_accounting_variables = ['Revenue', 'OperatingIncome', 'Equity(BV)', 'ShortTermDebt(BV)', 'LongTermDebt(BV)', 'LongTermLease(BV)', \
                                 'Debt(BV)', 'Cash',  'Tax', 'NetIncome']
...
def find_latest_tuple_by_string(data_list, search_string):
    """
    ...
    Args:
        ...
        search_string (str): The string value to search for.

    Returns:
        tuple or None: The tuple with the matching string and the latest datetime,
...
for report_date_dt, company_main_list in all_extracted_facts.items():
    ...
    if report_date_str in initialized_financial_df.columns:
        # Revenue Filling 
        revenue = find_latest_tuple_by_string(company_main_list, "Revenues")[1]
