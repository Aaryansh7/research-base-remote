...
import { fetchInventoryTurnoverRatio } from '../../api/ratios';
// Assuming you create a LoadingSpinner component
// import LoadingSpinner from '../../components/LoadingSpinner/LoadingSpinner';


const InventoryTurnoverChart = ({ selectedCompanyTicker, selectedCompanyName }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const result = await fetchInventoryTurnoverRatio(selectedCompanyTicker); // Call the centralized API function
        ...
        if (result.status === "success" && result.data) {
          ...
          setError(result.message || "Failed to load inventory turnover data.");
        }
      } catch (err) {
        console.error("Failed to fetch inventory turnover data:", err);
        setError(`Failed to load Inventory Turnover: ${err.message || 'Network error'}.`);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <div className="stats-box loading">
        ...
        <p>Loading Inventory Turnover data...</p>
      </div>
    );
  }
  ...
  if (!graphData || graphData.length === 0) {
    return (
      <div className="stats-box">
        <p>No Inventory Turnover data available for {selectedCompanyName || 'this company'}.</p>
      </div>
    );
  }
  ...
  return (
    <div className="stats-box">
      <h2>Inventory Turnover Trend for {selectedCompanyName || 'Selected Company'}</h2>
      <div style={{ width: '100%', height: 300 }}>
        <ResponsiveContainer>
          ...
          >
            ...
            <Legend />
            <Line type="monotone" dataKey="value" stroke="#8884d8" activeDot={{ r: 8 }} name="Inventory Turnover" />
          </LineChart>
        </ResponsiveContainer>
      </div>
      {stats && (
        <div className="revenue-statistics mt-4 p-4 bg-gray-100 rounded-md shadow-inner">
          ...
          <p className="text-gray-600"><strong>Average Inventory Turnover:</strong> {stats.average_margin}</p>
          <p className="text-gray-600"><strong>Standard Deviation Inventory Turnover:</strong> {stats.std_dev_margin}</p>
          <p className="text-sm text-gray-500 mt-2">
            Note: This graph displays the Inventory Turnover over available periods.
          </p>
        </div>
      )}
    </div>
