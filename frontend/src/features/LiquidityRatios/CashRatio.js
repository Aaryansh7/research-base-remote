...
import { fetchCashRatio } from '../../api/ratios';
// Assuming you create a LoadingSpinner component
// import LoadingSpinner from '../../components/LoadingSpinner/LoadingSpinner';


const CashRatioChart = ({ selectedCompanyTicker, selectedCompanyName }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const result = await fetchCashRatio(selectedCompanyTicker); // Call the centralized API function
  ...
  if (!graphData || graphData.length === 0) {
    return (
      <div className="stats-box">
        <p>No Cash Ratio data available for {selectedCompanyName || 'this company'}.</p>
      </div>
    );
  }
  ...
  return (
    <div className="stats-box">
      <h2>Cash Ratio Trend for {selectedCompanyName || 'Selected Company'}</h2>
      <div style={{ width: '100%', height: 300 }}>
        <ResponsiveContainer>
          ...
          >
            ...
            <Legend />
            <Line type="monotone" dataKey="value" stroke="#8884d8" activeDot={{ r: 8 }} name="Cash ratio" />
          </LineChart>
        </ResponsiveContainer>
      </div>
      {stats && (
        <div className="revenue-statistics mt-4 p-4 bg-gray-100 rounded-md shadow-inner">
          ...
          <p className="text-gray-600"><strong>Average Cash Ratio:</strong> {stats.average_margin}</p>
          <p className="text-gray-600"><strong>Standard Deviation Cash Ratio:</strong> {stats.std_dev_margin}</p>
          <p className="text-sm text-gray-500 mt-2">
            Note: This graph displays the Cash ratio over available periods.
          </p>
        </div>
      )}
    </div>
